...
from cs336_basics.multihead_self_attention import MHA
...
def run_embedding(
    ...
    embed_layer.load_state_dict({"W_embed": weights})
    out = embed_layer(token_ids)
    return out
...
def run_multihead_self_attention_with_rope(
    ...
    Returns:
        ...
        implementation with the given QKV projection weights and input features.
    """
    mha = MHA(d_model=d_model, num_heads=num_heads, theta=theta, max_seq_len=max_seq_len)
    mha.load_state_dict({
        "W_Q.W": q_proj_weight,
        "W_K.W": k_proj_weight,
        "W_V.W": v_proj_weight,
...
def run_rmsnorm(
    ...
    norm_layer.load_state_dict({"g": weights})
    out = norm_layer(in_features)
    return out
